...
static void assert_response(ldns_pkt *reply) {
    ASSERT_EQ(1, ldns_pkt_ancount(reply))
                                ...
                                << ldns_pkt_ancount(reply);
    auto first_rr = ldns_rr_list_rr(ldns_pkt_answer(reply), 0);

    ASSERT_EQ(LDNS_RR_TYPE_A, ldns_rr_get_type(first_rr))
                                << "DNS upstream returned wrong answer type instead of A"
