...
public:
    struct Params {
        ...
        const UpstreamFactoryConfig
                &upstream_config;       // configuration of the upstream factory which creates resolving upstream
...
struct ErrorCodeToString<dns::Bootstrapper::BootstrapperError> {
    std::string operator()(dns::Bootstrapper::BootstrapperError e) {
        switch (e) {
        case decltype(e)::AE_NO_VALID_RESOLVERS:
            return "Failed to create any resolver";
        case decltype(e)::AE_EMPTY_LIST:
            return "Empty bootstrap list";
        case decltype(e)::AE_RESOLVE_FAILED:
            return "Failed to resolve host";
        case decltype(e)::AE_TEMPORARY_DISABLED:
