...
public:
    struct on_read_callback {
         ...
         */
        bool (* func)(void *arg, uint8_view data);
     ...
     * @return some error if failed
     */
    [[nodiscard]] std::optional<socket::error> send(uint8_view data);
     ...
     * @return some error if failed
     */
    [[nodiscard]] std::optional<socket::error> send_dns_packet(uint8_view data);
