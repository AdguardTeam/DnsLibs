...
namespace ag::file {
     ...
     */
    int read(handle f, char *buf, size_t size);
     ...
     * @return     Number of read bytes (<0 in case of error)
     */
    int pread(handle f, char *buf, size_t size, size_t pos);
     ...
     */
    int write(handle f, const void *buf, size_t size);
     ...
     */
    int set_position(handle f, size_t pos);
    ...
    /**
     * @brief      Apply user function to each line in file while user function returns true
     *             or eof not met
     * @param[in]  f       file handle
     ...
     *             <0 otherwise
     */
    int for_each_line(handle f, line_action action, void *arg);
