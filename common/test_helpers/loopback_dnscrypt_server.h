...
public:
    ...
    //
    // start() binds UDP to :0, then binds TCP to that same port. A concurrent
    // test server (or a lingering TIME_WAIT socket) can take the TCP port in
    // the gap between the two binds, and then the TCP bind() fails with
    // EADDRINUSE. start() then selects a new ephemeral port a bounded number
    // of times. Same retry as in LoopbackDnsServer::start().
    //
    // Fail-fast: any other socket()/bind()/listen() failure (or a port that
    // resolves to 0) records a test failure via ADD_FAILURE (not ASSERT_*: those
    // expand to `co_return` and would turn this void method into a coroutine),
    // resets the partially-opened sockets, and returns without starting any
    // worker thread or flipping m_running — so stop()/destruction never join a
    // non-existent thread. Mirrors the defensive pattern in
    // LoopbackTlsServer::start().
    ...
    void start() {
        ...
        for (int attempt = 0; attempt < MAX_BIND_ATTEMPTS; ++attempt) {
            ...
            // either retrying with a new ephemeral port or returning.
            reset_start_state();
        ...
        ADD_FAILURE() << "LoopbackDnscryptServer: TCP bind() failed after " << MAX_BIND_ATTEMPTS << " attempts";
    }
...
private:
    ...
    // ephemeral-port race (close + select a new port); FATAL means a
    // non-retryable failure (ADD_FAILURE already recorded).
    ...
    // One bind attempt for the configured transports. Binds UDP to :0 (the OS
    // assigns P), then binds TCP to P. Returns RETRY when the TCP bind() to P
    // collided with a concurrent test server / lingering socket.
