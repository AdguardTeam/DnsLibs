...
static bool configure_tun_interface(
        std::string_view tun_name, std::string_view local_addr, std::string_view remote_addr) {
...
int main(int argc, char *argv[]) {
    ...
    dbglog(g_logger, "TUN interface configured successfully. Local address: {}. Remote address: {}", local_addr,
            remote_addr);
    ...
    TunListener::RequestCallback request_callback = [&proxy](
                                                            ag::Uint8View request, TunListener::Completion completion) {
    ...
    if (external_mode) {
        ...
        tun_result = tun_listener.init(-1, 0, request_callback, [tun_fd](ag::Uint8View packet) {
    ...
    if (external_mode) {
        ...
        while (keep_running) {
            ...
            if (nread > 0) {
                ...
                ag::Packet pkt{.data = packet_copy->data(),
                ...
                ag::Packets packets{.data = &pkt, .size = 1};
