...
static size_t truncate_loop(const ldns_rr_list *rrs, int section, ldns_rbtree_t *compression,
                            size_t max_size, size_t &cur_size) {
...
bool ag::ldns_pkt_truncate(ldns_pkt *pkt, uint16_t max_size) {
    if (max_size < 512) {
        max_size = 512;
    }
    ...
    // Free tree
    ldns_traverse_postorder(compression, [](ldns_rbnode_t *node, void *arg) {
        ldns_rdf_deep_free((ldns_rdf *)node->key);
        LDNS_FREE(node);
    }, nullptr);
    ldns_rbtree_free(compression);

    bool truncated = (qdcount != ldns_pkt_qdcount(pkt))
                     || (ancount != ldns_pkt_ancount(pkt))
                     || (nscount != ldns_pkt_nscount(pkt))
                     || (arcount != ldns_pkt_arcount(pkt));
