...
std::vector<uint8_t> dns_forwarder::handle_message_internal(Uint8View message, const dns_message_info *info,
    ...
    if (this->settings->block_ipv6 && LDNS_RR_TYPE_AAAA == type) {
        ...
        auto raw_blocking_response = apply_filter(
                { .domain = normalized_domain, .rr_type = question_rr_type(request), },
                request, nullptr,
                event,
                effective_rules,
                fallback_only,
...
std::optional<Uint8Vector> dns_forwarder::apply_cname_filter(const ldns_rr *cname_rr,
    ...
    return apply_filter(
            { .domain = cname, .rr_type = LDNS_RR_TYPE_CNAME, },
            request, response,
            event,
            last_effective_rules,
            fallback_only
    );
}
...
std::optional<Uint8Vector> dns_forwarder::apply_ip_filter(const ldns_rr *rr,
    ...
    return apply_filter(
            { .domain = addr_str, .rr_type = ldns_rr_get_type(rr) },
            request, response,
            event,
            last_effective_rules,
            fallback_only
...
std::optional<Uint8Vector> dns_forwarder::apply_filter(
    ...
    auto rules = this->filter.match(this->filter_handle, match);
...
void dns_forwarder::remove_ech_svcparam(ldns_pkt *response) {
    for (size_t i = 0; i < ldns_pkt_ancount(response); ++i) {
        ...
        if (auto type = ldns_rr_get_type(rr);
                (type != LDNS_RR_TYPE_SVCB && type != LDNS_RR_TYPE_HTTPS)
                || (ldns_rr_rd_count(rr) != 3)) {
            continue;
        }
        ...
        while (params_tail.size() >= sizeof(key)) {
            ...
            if (key == LDNS_SVCPARAM_KEY_ECHCONFIG) {
                dbglog_fid(logger, response, "Removing ECH parameters from SVCB/HTTPS RR");
