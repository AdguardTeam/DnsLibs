...
struct applicable_dnsrewrite_rules {
...
static std::optional<ldns_pkt_rcode> parse_rcode(const std::string &part, ag::Logger *log) {
    ...
    if (std::any_of(part.begin(), part.end(), [] (char c) -> bool { return ::islower(c); })) {
...
static std::optional<rule_utils::dnsrewrite_svcb_value> parse_svcb(std::string_view str, ag::Logger *log) {
    ...
    rule_utils::dnsrewrite_svcb_value value = {};
...
static std::pair<bool, std::optional<rule_utils::dnsrewrite_info>> parse_parameters(
    ...
    std::vector parts = !params.empty()
                        ? ag::utils::split_by(params, PARTS_DELIM, true) : std::vector<std::string_view>{};
    if (!parts.empty()
            && parts.size() != SHORTHAND_PARTS_NUM
            && parts.size() != FULLY_QUALIFIED_PARTS_NUM) {
        ru_dbglog(log, "Wrong number of parts: {}", parts.size());
        return { false, std::nullopt };
    }

    std::optional<rule_utils::dnsrewrite_info> info;
    auto get_info =
            [&info] () -> rule_utils::dnsrewrite_info & {
                if (!info.has_value()) {
                    info.emplace();
                }
                return info.value();
    ...
    case FULLY_QUALIFIED_PARTS_NUM: {
        for (size_t i = 0; i < FULLY_QUALIFIED_PARTS_NUM; ++i) {
            ...
            case DRPI_RCODE:
                ...
                if (auto rcode = parse_rcode(std::string(parts[i]), log);
                        rcode.has_value()) {
                    get_info().rcode = rcode.value();
                    continue;
                }
...
static bool parse_value_by_rrtype(rule_utils::dnsrewrite_info &info,
    ...
    case LDNS_RR_TYPE_PTR: {
        ...
        bool is_valid_pattern = !match_info.has_wildcard
                && !match_info.is_regex_rule
...
bool rule_utils::parse_dnsrewrite_modifier(rule &rule, std::string_view params_str,
    ...
    const auto &content = std::get<ag::dnsfilter::adblock_rule_info>(rule.public_part.content);
    if (!info.has_value()) {
        if (!content.props.test(ag::dnsfilter::DARP_EXCEPTION)) {
            info.emplace();
        }
...
static bool has_higher_priority(const dnsfilter::rule *l, const dnsfilter::rule *r) {
    const auto &linfo = std::get<dnsfilter::adblock_rule_info>(l->content).params->dnsrewrite.value();
    const auto &rinfo = std::get<dnsfilter::adblock_rule_info>(r->content).params->dnsrewrite.value();
...
static applicable_dnsrewrite_rules get_applicable_rules(const std::vector<const dnsfilter::rule *> &rules) {
    using rule = dnsfilter::rule;

    applicable_dnsrewrite_rules applicable_rules;
    applicable_rules.blocking.reserve(rules.size());
    for (const rule *r : rules) {
        if (const auto &info = std::get<dnsfilter::adblock_rule_info>(r->content);
    ...
    for (auto i = applicable_rules.blocking.begin(); i != applicable_rules.blocking.end();) {
        auto found_exclusion = std::find_if(
                applicable_rules.exclusions.cbegin(), applicable_rules.exclusions.cend(),
                [blocking = *i] (const rule *exclusion) {
                    const auto &blocking_info = std::get<dnsfilter::adblock_rule_info>(blocking->content);
                    const auto &exclusion_info = std::get<dnsfilter::adblock_rule_info>(exclusion->content);
    ...
    if (applicable_rules.blocking.size() > 1) {
        const auto &info = std::get<dnsfilter::adblock_rule_info>(applicable_rules.blocking[0]->content)
...
dnsfilter::apply_dnsrewrite_result dnsfilter::apply_dnsrewrite_rules(
        const std::vector<const rule *> &rules) {
    using rule = dnsfilter::rule;

    applicable_dnsrewrite_rules applicable_rules = get_applicable_rules(rules);

    apply_dnsrewrite_result result;
    ...
    for (const rule *r : applicable_rules.blocking) {
        const auto &rule_info = std::get<dnsfilter::adblock_rule_info>(r->content);
        const rule_utils::dnsrewrite_info &dnsrewrite = rule_info.params->dnsrewrite.value();

        auto &rewr_info = result.rewritten_info;
        if (!rewr_info.has_value()) {
            rewr_info.emplace(apply_dnsrewrite_result::rewrite_info{});
        ...
        case LDNS_RR_TYPE_AAAA:
            ...
            break;
        case LDNS_RR_TYPE_MX: {
            const auto &mx = std::get<rule_utils::dnsrewrite_mx_value>(dnsrewrite.parsed_value);
        ...
        case LDNS_RR_TYPE_HTTPS:
        case LDNS_RR_TYPE_SVCB: {
            const auto &svcb = std::get<rule_utils::dnsrewrite_svcb_value>(dnsrewrite.parsed_value);
