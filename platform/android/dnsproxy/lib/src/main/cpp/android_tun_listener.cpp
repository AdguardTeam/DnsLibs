...
extern "C" JNIEXPORT void JNICALL Java_com_adguard_dnslibs_proxy_DnsTunListener_00024NativeReplyHandler_nativeSendReply(
...
AndroidTunListener::AndroidTunListener(JavaVM *vm)
    ...
    jclass callback_class =
            (m_jclasses.request_callback_class = GlobalRef(vm, env->FindClass(FQN_DNSTUNLISTENER_REQUEST_CALLBACK)))
                    .get();
    m_request_callback_methods.on_request =
            env->GetMethodID(callback_class, "onRequest", "([BL" FQN_DNSTUNLISTENER_REPLY_HANDLER ";)V");

    // Get NativeReplyHandler class and constructor
    jclass handler_class = (m_jclasses.native_reply_handler_class =
                                    GlobalRef(vm, env->FindClass(FQN_DNSTUNLISTENER_NATIVE_REPLY_HANDLER)))
                                   .get();
...
TunListener::RequestCallback AndroidTunListener::marshal_request_callback(JNIEnv *env, jobject request_callback) {
    ...
    return [this, vm](Uint8View request, TunListener::Completion completion) {
        ...
        jni::LocalRef<jobject> reply_handler{scoped_env.get(),
                scoped_env->NewObject(m_jclasses.native_reply_handler_class.get(), m_native_reply_handler_methods.ctor,
                        (jlong) this, (jlong) completion_id)};

        // Call Java method with request and ReplyHandler object
        scoped_env->CallVoidMethod(m_request_callback.get(), m_request_callback_methods.on_request, java_request.get(),
                reply_handler.get());
