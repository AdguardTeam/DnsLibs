...
class DnsLibsConan(ConanFile):
    ...
    def generate(self):
        ...
        # Bake the package version: the conan source is fetched by tag (no .git),
        # so CMake's git-describe fallback cannot see it — pass it explicitly.
        # configure_file then generates version.h with this value during build().
        tc.cache_variables["DNS_LIBS_VERSION"] = str(self.version)
        tc.generate()
    ...
    def package(self):
        ...
        # version.h is not in the source tree (only version.h.in); ship the copy
        # generated by configure_file during the build, baked with self.version.
        copy(self, "version.h", src=self.build_folder, dst=join(self.package_folder, "include", "dns", "common"), keep_path=False)
